...
func _on_search_text_changed(text: String) -> void:
	if text.is_empty():
		for item in _all_file_items:
			item.visible = true
		return
	
	var search_lower := text.to_lower()
	for item in _all_file_items:
		var filename := item.get_text(0).to_lower()
		item.visible = filename.contains(search_lower)
...
func _scan_directory(path: String, parent: TreeItem) -> void:
	...
	for file in dir.get_files():
		if file.ends_with(".gdshaderinc") or file.ends_with(".gdshader") or file.ends_with(".glsl"):
			...
			_all_file_items.append(item)
			if file.ends_with(".gdshaderinc"):
				item.set_icon(0, _get_icon("ShaderInclude"))
			elif file.ends_with(".gdshader"):
				item.set_icon(0, _get_icon("Shader"))
			elif file.ends_with(".glsl"):
...
func create_include_from_template(template_path: String, target_path: String, data: Dictionary) -> void:
	print(template_path)
	print(target_path)
...
func _on_tree_column_title_clicked(column: int, click_count: int) -> void:
	pass
